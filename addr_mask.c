 ...
 * @purpose    Generate entropy using multiple kernel sources when BCrypt unavailable
 ...
 */
static VOID
MonAddrMaskGenerateEntropyFallback(
    _Out_writes_bytes_(16) PUCHAR OutputBuffer
)
{
...
NTSTATUS MonAddrMaskInitialize(VOID)
{
    ...
    if (!NT_SUCCESS(status)) {
         ...
         * 2. We combine multiple entropy sources including ASLR-randomized addresses
        ...
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
            "[WIN11MON][MASK] BCrypt failed (0x%08X), using enhanced fallback RNG\n",
            status);
...
{
    return (MON_ADDRESS_MASK_POLICY)InterlockedCompareExchange(
        &g_MaskPolicy,
        g_MaskPolicy,
        g_MaskPolicy
    );
