...
#define IOCTL_MONITOR_INTERCEPT_VALIDATE    CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x20, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_MONITOR_INTERCEPT_SET_POLICY  CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x21, METHOD_BUFFERED, FILE_WRITE_ACCESS)
#define IOCTL_MONITOR_INTERCEPT_GET_POLICY  CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x22, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_MONITOR_INTERCEPT_GET_STATS   CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x23, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_MONITOR_INTERCEPT_RESET_STATS CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x24, METHOD_BUFFERED, FILE_WRITE_ACCESS)
#define IOCTL_MONITOR_INTERCEPT_ENABLE      CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x25, METHOD_BUFFERED, FILE_WRITE_ACCESS)
#define IOCTL_MONITOR_INTERCEPT_ADD_BL      CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x26, METHOD_BUFFERED, FILE_WRITE_ACCESS)
#define IOCTL_MONITOR_INTERCEPT_REMOVE_BL   CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x27, METHOD_BUFFERED, FILE_WRITE_ACCESS)
#define IOCTL_MONITOR_INTERCEPT_GET_BL      CTL_CODE(WIN11MON_DEVICE_TYPE, WIN11MON_IOCTL_BASE + 0x28, METHOD_BUFFERED, FILE_READ_ACCESS)
 ...
 * Reference: https://windows-internals.com/i-o-rings-when-one-i-o-operation-is-not-enough/
 *-------------------------------------------------------------------------*/
 ...
 *-------------------------------------------------------------------------*/
typedef NTSTATUS (NTAPI *PFN_NtSubmitIoRing)(
 ...
 *-------------------------------------------------------------------------*/
static HANDLE
GetDriverHandleFromClient(
    _In_ HWIN11MON Handle
)
{
    /* The HWIN11MON is an opaque pointer to a structure containing the driver handle */
    /* For simplicity, assume first field is the driver handle */
    if (Handle == NULL) {
        return INVALID_HANDLE_VALUE;
    }

    /* Access the internal structure - this depends on win11mon_client.c implementation */
 ...
 *-------------------------------------------------------------------------*/
static PFN_NtSubmitIoRing
ResolveNtSubmitIoRing(VOID)
{
...
)
{
    ...
    PWIN11MON_INTERCEPT_REQUEST request = (PWIN11MON_INTERCEPT_REQUEST)
        HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, reqBufferSize);
    ...
    hr = SendIoctl(
        ...
        requestSize,
        Response,
        sizeof(WIN11MON_INTERCEPT_RESPONSE),
        &bytesReturned
    );
...
)
{
    ...
    *BufferSize = 0;
    ...
    __try {
        ...
        PWIN11MON_SERIALIZED_SQE buffer = (PWIN11MON_SERIALIZED_SQE)
...
CallOriginal:
    ...
    if (g_InterceptState.OriginalNtSubmitIoRing != NULL) {
        status = g_InterceptState.OriginalNtSubmitIoRing(
            IoRingHandle,
            Flags,
            WaitOperations,
            Timeout
...
)
{
    ...
    *(PVOID*)(&simpleHook[2]) = Detour;
    ...
    PVOID trampoline = VirtualAlloc(
        NULL,
        hookSize + 14,  /* Original bytes + jump back */
...
RemoveInlineHook(VOID)
{
    ...
    CopyMemory(g_InterceptState.HookTarget,
               g_InterceptState.OriginalBytes,
               g_InterceptState.OriginalBytesSize);
...
)
{
    ...
    return SendIoctl(
        ...
        Policy,
        sizeof(WIN11MON_INTERCEPT_POLICY),
        NULL,
        0,
        NULL
    );
...
)
{
    ...
    return SendIoctl(
        ...
        0,
        Stats,
        sizeof(WIN11MON_INTERCEPT_STATS),
        &bytesReturned
    );
}
...
)
{
    ...
    return SendIoctl(
        IOCTL_MONITOR_INTERCEPT_ADD_BL,
        &request,
        sizeof(request),
        NULL,
        0,
...
)
{
    ...
    return SendIoctl(
        IOCTL_MONITOR_INTERCEPT_REMOVE_BL,
        &ProcessId,
        sizeof(ProcessId),
        NULL,
        0,
...
)
{
    ...
    hr = Win11MonInterceptGetBlacklist(Handle, entries, WIN11MON_INTERCEPT_MAX_BLACKLIST, &entryCount);
    if (FAILED(hr)) {
        return hr;
    }
...
)
{
    ...
    HRESULT hr = Win11MonInterceptGetBlacklist(
        Handle,
        entries,
        WIN11MON_INTERCEPT_MAX_BLACKLIST,
        &entryCount
    );
...
)
{
    ...
    *EntryCount = 0;
    ...
    HRESULT hr = SendIoctl(
        IOCTL_MONITOR_INTERCEPT_GET_BL,
        &MaxEntries,
        sizeof(MaxEntries),
        Buffer,
        bufferSize,
...
)
{
    ...
    hr = InstallInlineHook(
        (PVOID)pfnOriginal,
        (PVOID)HookedNtSubmitIoRing,
        &trampoline
    );
