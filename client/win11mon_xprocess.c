...
static const WCHAR *g_SeverityStrings[] = {L"Info", L"Low", L"Medium", L"High", L"Critical"};
...
static const WCHAR *g_IntegrityStrings[] = {L"Untrusted", L"Low",    L"Medium",   L"Medium+",
                                            L"High",      L"System", L"Protected"};

/*--------------------------------------------------------------------------
 * Internal Helper
 *-------------------------------------------------------------------------*/
static DWORD XpIoctl(_In_ HANDLE hDevice, _In_ DWORD IoCtlCode, _In_opt_ LPVOID pInput,
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpGetSharedObjects(HANDLE hDevice, PVOID pBuffer,
                                                               DWORD BufferSize,
                                                               DWORD *pBytesWritten) {
  DWORD err;
  ...
  *pBytesWritten = 0;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_SHARED, NULL, 0, pBuffer, BufferSize, pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpGetProcessTree(HANDLE hDevice, PVOID pBuffer,
                                                             DWORD BufferSize,
                                                             DWORD *pBytesWritten) {
  DWORD err;
  ...
  *pBytesWritten = 0;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_TREE, NULL, 0, pBuffer, BufferSize, pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpScanSections(HANDLE hDevice, DWORD ProcessId,
  ...
  *pBytesWritten = 0;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_SCAN_SECTIONS, &input, sizeof(input), pBuffer, BufferSize,
                pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpGetAlerts(HANDLE hDevice, PVOID pBuffer,
  ...
  *pBytesWritten = 0;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_ALERTS, NULL, 0, pBuffer, BufferSize, pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpGetStats(HANDLE hDevice, PWIN11MON_XP_STATS pStats) {
  DWORD bytesReturned;
  DWORD err;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_STATS, NULL, 0, pStats, sizeof(*pStats),
                &bytesReturned);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpGetConfig(HANDLE hDevice,
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_CONFIG, NULL, 0, pConfig, sizeof(*pConfig),
                &bytesReturned);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpSetConfig(HANDLE hDevice,
  ...
  return XpIoctl(hDevice, IOCTL_MONITOR_XP_SET_CONFIG, (LPVOID)pConfig, sizeof(*pConfig), NULL, 0,
                 &bytesReturned);
}
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpScanNow(HANDLE hDevice) {
  ...
  return XpIoctl(hDevice, IOCTL_MONITOR_XP_SCAN_NOW, NULL, 0, NULL, 0, &bytesReturned);
}
...
_Use_decl_annotations_ const WCHAR *WINAPI Win11MonXpRuleIdToString(WIN11MON_XP_RULE_ID RuleId) {
...
_Use_decl_annotations_ const WCHAR *WINAPI Win11MonXpIntegrityToString(DWORD IntegrityLevel) {
