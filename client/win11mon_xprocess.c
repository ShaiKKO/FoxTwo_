...
#define IOCTL_MONITOR_XP_GET_SHARED                                           \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x50, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_XP_GET_TREE                                             \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x51, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_XP_SCAN_SECTIONS                                        \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x52, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_XP_GET_ALERTS                                           \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x53, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_XP_GET_STATS                                            \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x54, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_XP_GET_CONFIG                                           \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x55, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_XP_SET_CONFIG                                           \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x56, METHOD_BUFFERED, \
           FILE_WRITE_ACCESS)

#define IOCTL_MONITOR_XP_SCAN_NOW                                             \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x57, METHOD_BUFFERED, \
           FILE_WRITE_ACCESS)

/*--------------------------------------------------------------------------
 * Static String Tables
 *-------------------------------------------------------------------------*/
static const WCHAR* g_AlertTypeStrings[] = {L"None",
                                            L"Shared IoRing",
                                            L"Unrelated Sharing",
...
static const WCHAR* g_SeverityStrings[] = {L"Info", L"Low", L"Medium", L"High",
                                           L"Critical"};

static const WCHAR* g_RuleStrings[] = {L"Unknown",
...
static const WCHAR* g_IntegrityStrings[] = {L"Untrusted", L"Low",  L"Medium",
                                            L"Medium+",   L"High", L"System",
                                            L"Protected"};

/*--------------------------------------------------------------------------
 * Internal Helper
 *-------------------------------------------------------------------------*/
static DWORD XpIoctl(_In_ HANDLE hDevice, _In_ DWORD IoCtlCode,
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpGetSharedObjects(
    HANDLE hDevice, PVOID pBuffer, DWORD BufferSize, DWORD* pBytesWritten) {
  DWORD err;
  ...
  *pBytesWritten = 0;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_SHARED, NULL, 0, pBuffer,
                BufferSize, pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpGetProcessTree(
    HANDLE hDevice, PVOID pBuffer, DWORD BufferSize, DWORD* pBytesWritten) {
  DWORD err;
  ...
  *pBytesWritten = 0;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_TREE, NULL, 0, pBuffer,
                BufferSize, pBytesWritten);
...
Win11MonXpScanSections(HANDLE hDevice, DWORD ProcessId, PVOID pBuffer,
                       DWORD BufferSize, DWORD* pBytesWritten) {
  ULONG64 input;
  DWORD err;
  ...
  *pBytesWritten = 0;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_SCAN_SECTIONS, &input, sizeof(input),
                pBuffer, BufferSize, pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonXpGetAlerts(HANDLE hDevice,
                                                        PVOID pBuffer,
                                                        DWORD BufferSize,
                                                        DWORD* pBytesWritten) {
  DWORD err;
  ...
  *pBytesWritten = 0;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_ALERTS, NULL, 0, pBuffer,
                BufferSize, pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI
Win11MonXpGetStats(HANDLE hDevice, PWIN11MON_XP_STATS pStats) {
  DWORD bytesReturned;
  DWORD err;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_STATS, NULL, 0, pStats,
...
_Use_decl_annotations_ DWORD WINAPI
Win11MonXpGetConfig(HANDLE hDevice, PWIN11MON_XP_CONFIG pConfig) {
  DWORD bytesReturned;
  DWORD err;
  ...
  err = XpIoctl(hDevice, IOCTL_MONITOR_XP_GET_CONFIG, NULL, 0, pConfig,
                sizeof(*pConfig), &bytesReturned);
...
_Use_decl_annotations_ DWORD WINAPI
Win11MonXpSetConfig(HANDLE hDevice, const WIN11MON_XP_CONFIG* pConfig) {
  DWORD bytesReturned;
  ...
  return XpIoctl(hDevice, IOCTL_MONITOR_XP_SET_CONFIG, (LPVOID)pConfig,
                 sizeof(*pConfig), NULL, 0, &bytesReturned);
}

_Use_decl_annotations_ DWORD WINAPI Win11MonXpScanNow(HANDLE hDevice) {
  DWORD bytesReturned;

  return XpIoctl(hDevice, IOCTL_MONITOR_XP_SCAN_NOW, NULL, 0, NULL, 0,
                 &bytesReturned);
}
...
Win11MonXpAlertTypeToString(WIN11MON_XP_ALERT_TYPE AlertType) {
...
_Use_decl_annotations_ const WCHAR* WINAPI
Win11MonXpSeverityToString(WIN11MON_XP_SEVERITY Severity) {
...
_Use_decl_annotations_ const WCHAR* WINAPI
Win11MonXpRuleIdToString(WIN11MON_XP_RULE_ID RuleId) {
...
_Use_decl_annotations_ const WCHAR* WINAPI
Win11MonXpIntegrityToString(DWORD IntegrityLevel) {
