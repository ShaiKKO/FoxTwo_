...
#define IOCTL_MONITOR_MEM_SCAN_VAD                                            \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x40, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_MEM_GET_MDLS                                            \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x41, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_MEM_SCAN_PHYSICAL                                       \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x42, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_MEM_GET_SHARING                                         \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x43, METHOD_BUFFERED, \
           FILE_READ_ACCESS)

#define IOCTL_MONITOR_MEM_GET_STATS                                           \
  CTL_CODE(FILE_DEVICE_WIN11MON, WIN11MON_IOCTL_BASE + 0x44, METHOD_BUFFERED, \
           FILE_READ_ACCESS)
...
static const WCHAR* g_VadTypeStrings[] = {
    L"Private",    L"Mapped",     L"Image",  L"Physical",
    L"WriteWatch", L"LargePages", L"Rotate", L"Unknown"};

/*--------------------------------------------------------------------------
 * Internal Helper
 *-------------------------------------------------------------------------*/
static DWORD MemIoctl(_In_ HANDLE hDevice, _In_ DWORD IoCtlCode,
...
_Use_decl_annotations_ DWORD WINAPI Win11MonMemScanVad(
    HANDLE hDevice, DWORD ProcessId, PWIN11MON_VAD_SCAN_RESULT pResult) {
  ...
  err = MemIoctl(hDevice, IOCTL_MONITOR_MEM_SCAN_VAD, &input, sizeof(input),
                 pResult, sizeof(*pResult), &bytesReturned);
...
Win11MonMemScanVadDetailed(HANDLE hDevice, DWORD ProcessId, PVOID pBuffer,
...
_Use_decl_annotations_ DWORD WINAPI Win11MonMemGetMdls(HANDLE hDevice,
                                                       DWORD ProcessId,
                                                       PVOID pBuffer,
                                                       DWORD BufferSize,
                                                       DWORD* pBytesWritten) {
  ...
  *pBytesWritten = 0;
  ...
  err = MemIoctl(hDevice, IOCTL_MONITOR_MEM_GET_MDLS, &input, sizeof(input),
                 pBuffer, BufferSize, pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI Win11MonMemScanPhysical(
    HANDLE hDevice, DWORD ProcessId, PWIN11MON_PHYSICAL_SCAN_RESULT pResult) {
  MEM_SCAN_INPUT input;
  DWORD bytesReturned;
  DWORD err;
  ...
  err = MemIoctl(hDevice, IOCTL_MONITOR_MEM_SCAN_PHYSICAL, &input,
                 sizeof(input), pResult, sizeof(*pResult), &bytesReturned);
...
_Use_decl_annotations_ DWORD WINAPI
Win11MonMemGetSharing(HANDLE hDevice, DWORD ProcessId, PVOID pBuffer,
                      DWORD BufferSize, DWORD* pBytesWritten) {
  MEM_SCAN_INPUT input;
  DWORD err;
  ...
  *pBytesWritten = 0;
  ...
  err = MemIoctl(hDevice, IOCTL_MONITOR_MEM_GET_SHARING, &input, sizeof(input),
                 pBuffer, BufferSize, pBytesWritten);
...
_Use_decl_annotations_ DWORD WINAPI
Win11MonMemGetStats(HANDLE hDevice, PWIN11MON_MEM_STATS pStats) {
  DWORD bytesReturned;
  ...
  return MemIoctl(hDevice, IOCTL_MONITOR_MEM_GET_STATS, NULL, 0, pStats,
...
_Use_decl_annotations_ const WCHAR* WINAPI
Win11MonMemVadTypeToString(WIN11MON_VAD_TYPE VadType) {
...
_Use_decl_annotations_ DWORD WINAPI Win11MonMemProtectionToString(
    ULONG Protection, WCHAR* Buffer, DWORD BufferSize) {
