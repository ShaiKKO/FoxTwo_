...
struct McLayout
{
    ...
    McLayout() : SizeBytes(0),
                 OffsetType(0),
                 OffsetReserved(0),
                 OffsetSize(0),
                 OffsetReferenceCount(0),
                 OffsetFlags(0),
                 OffsetGlobalDataLink(0),
                 OffsetAddress(0),
                 OffsetLength(0),
                 OffsetAccessMode(0),
                 OffsetMdlRef(0),
...
static void
LogInfoVerbose(const McLayoutOptions& opts, const wchar_t* fmt, ...)
{
...
static bool
ReadImagePdbSignature(const std::wstring& imagePath, GUID* outGuid, DWORD* outAge)
{
...
static bool
ValidatePdbMatchesImage(const McLayoutOptions& opts,
                        const GUID& pdbGuid,
                        DWORD pdbAge)
{
    GUID imgGuid = {};
    ...
    if (!InlineIsEqualGUID(pdbGuid, imgGuid) || pdbAge != imgAge) {
        ...
        if (StringFromGUID2(pdbGuid, pdbGuidStr, static_cast<int>(sizeof(pdbGuidStr) / sizeof(wchar_t))) == 0) {
            pdbGuidStr[0] = L'\0';
        }
        if (StringFromGUID2(imgGuid, imgGuidStr, static_cast<int>(sizeof(imgGuidStr) / sizeof(wchar_t))) == 0) {
            imgGuidStr[0] = L'\0';
        }

        LogError(L"PDB does not match image. PDB GUID=%ls Age=%lu, Image GUID=%ls Age=%lu",
...
static bool
ReadImageVersion(const std::wstring& imagePath,
                 DWORD* maj, DWORD* min,
                 DWORD* build, DWORD* rev)
{
...
static HRESULT
InitializeDiaSession(const McLayoutOptions& opts,
                     ...
                     DWORD* outAge)
{
    if (ppSource == nullptr || ppSession == nullptr || ppGlobal == nullptr ||
        outGuid == nullptr || outAge == nullptr) {
        return E_INVALIDARG;
    }
    ...
    *outAge = 0;
    ...
    HRESULT hr = CoCreateInstance(CLSID_DiaSource, nullptr, CLSCTX_INPROC_SERVER,
                                  IID_IDiaDataSource, reinterpret_cast<void**>(&source));
    if (FAILED(hr)) {
...
static bool
ExtractFieldOffset(IDiaSymbol* pUdt, const wchar_t* fieldName, ULONG* outOffset)
{
...
static bool
ExtractMcLayout(IDiaSession* session,
                IDiaSymbol* global,
                const McLayoutOptions& opts,
                McLayout& layout)
{
    ...
    IDiaEnumSymbols* pEnum = nullptr;
    HRESULT hr = global->findChildren(SymTagUDT,
...
static bool
WriteHeader(const McLayoutOptions& opts, const McLayout& layout)
{
...
static bool
WriteJson(const McLayoutOptions& opts, const McLayout& layout)
{
    ...
    std::fwprintf(f, L"  \"build\": { \"major\": %lu, \"minor\": %lu, \"number\": %lu, \"revision\": %lu },\n",
                  static_cast<unsigned long>(layout.FileMajor),
                  static_cast<unsigned long>(layout.FileMinor),
                  static_cast<unsigned long>(layout.FileBuild),
                  static_cast<unsigned long>(layout.FileRevision));
...
{
    std::fwprintf(stderr,
        L"Usage: mc_layout_gen.exe --image <ntoskrnl.exe> --pdb <ntkrnlmp.pdb> --out-header <path> [options]\n"
        L"Options:\n"
        L"  --out-json <path>       Optional JSON metadata output\n"
        L"  --symbol-name <name>    Target UDT name (default: _IOP_MC_BUFFER_ENTRY)\n"
        L"  --era <name>            Layout era label (e.g. WIN11_ERA_226XX)\n"
        L"  --verbose               Enable verbose logging to stderr\n");
}

static bool
ParseArguments(int argc, wchar_t** argv, McLayoutOptions& opts)
{
...
static int
RunExtractor(const McLayoutOptions& opts)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        // COM already initialized in a different mode; proceed without owning lifetime.
    } else {
    ...
    if (!ReadImageVersion(opts.ImagePath,
                          &layout.FileMajor,
                          &layout.FileMinor,
                          &layout.FileBuild,
                          &layout.FileRevision)) {
        LogInfoVerbose(opts, L"Warning: unable to read image version metadata from %ls", opts.ImagePath.c_str());
    }
