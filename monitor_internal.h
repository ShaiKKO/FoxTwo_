...
#define MonAllocatePoolPaged(Size, Tag) \
    ExAllocatePool2(POOL_FLAG_PAGED, (Size), (Tag))

#define MonAllocatePoolNonPaged(Size, Tag) \
...
#define MON_IS_KERNEL_ADDRESS(addr) \
    ((addr) != NULL && (ULONG_PTR)(addr) >= MmUserProbeAddress)
...
/* pool_tracker.c */
typedef NTSTATUS (*PFN_MON_ANALYZE_REGARRAY)(
    _In_ PVOID ArrayVirtualAddress,
...
NTSTATUS MonAnalyzeIoRingRegArray(
    _In_ PVOID ArrayVirtualAddress,
    _In_ SIZE_T ArrayByteLength);
