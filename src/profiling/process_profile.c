...
static VOID MonProfileLogAnomalyEvent(_In_ PMON_PROCESS_PROFILE Profile,
                                      _In_ MON_ANOMALY_RULE_ID RuleId, _In_ ULONG ActualValue,
...
_Use_decl_annotations_ NTSTATUS MonProfileInitialize(VOID) {
  NTSTATUS status;
...
_Use_decl_annotations_ VOID MonProfileShutdown(VOID) {
  PLIST_ENTRY entry;
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
             ...
             g_ProfileState.TotalProfilesCreated, g_ProfileState.TotalProfilesDestroyed);
}

_Use_decl_annotations_ BOOLEAN MonProfileIsInitialized(VOID) {
...
_Use_decl_annotations_ NTSTATUS MonProfileCreate(ULONG ProcessId, PCWSTR ImageName) {
  PMON_PROCESS_PROFILE profile;
  ...
  profile = (PMON_PROCESS_PROFILE)ExAllocatePoolWithTag(NonPagedPoolNx, sizeof(MON_PROCESS_PROFILE),
                                                        MON_PROFILE_TAG);
  ...
  if (ImageName != NULL) {
    RtlStringCchCopyW(profile->ImageName, RTL_NUMBER_OF(profile->ImageName), ImageName);
  }
...
_Use_decl_annotations_ BOOLEAN MonProfileDestroy(ULONG ProcessId) {
  ...
  if (found && profile != NULL) {
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
               ...
               profile->TotalOperations, profile->AnomalyScore);
...
static PMON_PROCESS_PROFILE MonProfileFindByPidLocked(_In_ ULONG ProcessId) {
  ...
  for (entry = g_ProfileState.ProfileList.Flink; entry != &g_ProfileState.ProfileList;
       entry = entry->Flink) {
    profile = CONTAINING_RECORD(entry, MON_PROCESS_PROFILE, ListEntry);
...
_Use_decl_annotations_ PMON_PROCESS_PROFILE MonProfileGetByPid(ULONG ProcessId) {
  PMON_PROCESS_PROFILE profile;
...
_Use_decl_annotations_ VOID MonProfileRecordOperation(ULONG ProcessId, ULONG OpCode,
                                                      ULONG BufferSize) {
  PMON_PROCESS_PROFILE profile;
  LARGE_INTEGER currentTime;
  ...
  if (profile != NULL) {
    ...
    if (BufferSize > 0) {
      if (OpCode == 3) { /* RegisterBuffers */
        InterlockedAdd64((volatile LONG64 *)&profile->TotalBufferBytesRegistered, BufferSize);
...
_Use_decl_annotations_ VOID MonProfileRecordHandle(ULONG ProcessId, BOOLEAN Created) {
  PMON_PROCESS_PROFILE profile;
...
_Use_decl_annotations_ VOID MonProfileRecordViolation(ULONG ProcessId) {
  PMON_PROCESS_PROFILE profile;
...
static VOID MonProfileLogAnomalyEvent(_In_ PMON_PROCESS_PROFILE Profile,
  ...
  MonRingBufferWrite(MonEvent_ProcessAnomalyDetected, &eventData, sizeof(eventData));

  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
             "[WIN11MON][PROFILE] Anomaly PID=%lu Rule=%ws Value=%lu>%lu Score=%ld\n",
             Profile->ProcessId, eventData.RuleName, ActualValue, Threshold, Profile->AnomalyScore);
}
...
_Use_decl_annotations_ NTSTATUS MonProfileGetSummary(ULONG ProcessId,
...
_Use_decl_annotations_ NTSTATUS MonProfileEnumerate(PMON_PROFILE_SUMMARY Buffer, ULONG MaxCount,
                                                    ULONG *ActualCount) {
  PLIST_ENTRY entry;
  PMON_PROCESS_PROFILE profile;
  ULONG count = 0;
  ...
  *ActualCount = 0;
  ...
  for (entry = g_ProfileState.ProfileList.Flink;
       entry != &g_ProfileState.ProfileList && count < MaxCount; entry = entry->Flink) {
    profile = CONTAINING_RECORD(entry, MON_PROCESS_PROFILE, ListEntry);

    Buffer[count].Size = sizeof(MON_PROFILE_SUMMARY);
    Buffer[count].ProcessId = profile->ProcessId;
    RtlCopyMemory(Buffer[count].ProcessName, profile->ImageName, sizeof(Buffer[count].ProcessName));
...
_Use_decl_annotations_ NTSTATUS MonProfileExportFeatures(ULONG ProcessId,
  ...
  if (profile->TotalBuffersRegistered > 0) {
    Features->AvgBufferSizeKB =
        (float)(profile->TotalBufferBytesRegistered / profile->TotalBuffersRegistered) / 1024.0f;
  }
  ...
  if (profile->TotalOperations > 0) {
    Features->ViolationRate =
        (float)profile->ViolationCount * 1000.0f / (float)profile->TotalOperations;
  }
...
_Use_decl_annotations_ VOID MonProfileGetStats(PMON_PROFILE_STATS Stats) {
  if (Stats == NULL) {
    return;
  }
...
_Use_decl_annotations_ NTSTATUS MonProfileSetConfig(PMON_PROFILE_CONFIG Config) {
...
_Use_decl_annotations_ VOID MonProfileGetConfig(PMON_PROFILE_CONFIG Config) {
  if (Config == NULL) {
    return;
  }
...
_Use_decl_annotations_ VOID MonProfileResetAll(VOID) {
  PLIST_ENTRY entry;
  PMON_PROCESS_PROFILE profile;
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][PROFILE] All profiles reset\n");
}
