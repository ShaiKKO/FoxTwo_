...
#define MON_PROFILE_MAX_PROCESSES       1024        /* Max simultaneous profiled processes */
...
MonProfileGetSummary(
    _In_ ULONG ProcessId,
    _Out_ PMON_PROFILE_SUMMARY Summary
);
...
NTSTATUS
MonProfileEnumerate(
    _Out_writes_to_(MaxCount, *ActualCount) PMON_PROFILE_SUMMARY Buffer,
    _In_ ULONG MaxCount,
...
MonProfileExportFeatures(
    _In_ ULONG ProcessId,
    _Out_ PMON_ML_FEATURE_VECTOR Features
);
...
VOID
MonProfileGetStats(
    _Out_ PMON_PROFILE_STATS Stats
);
...
NTSTATUS
MonProfileSetConfig(
    _In_ PMON_PROFILE_CONFIG Config
);
...
VOID
MonProfileGetConfig(
    _Out_ PMON_PROFILE_CONFIG Config
);
