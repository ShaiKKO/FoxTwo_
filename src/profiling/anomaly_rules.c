...
static const BUILTIN_RULE_DEF g_BuiltinRules[] = {
    {MonAnomalyRule_HighOpsFrequency, L"HighOpsFrequency", 1000, 5, MonSeverity_Medium, 20,
     "T1499"},

    {MonAnomalyRule_LargeBufferRegistration, L"LargeBuffer", 100 * 1024 * 1024, 0,
     MonSeverity_Medium, 15, "T1068"},

    {MonAnomalyRule_RapidHandleCreation, L"RapidHandles", 10, 1, MonSeverity_Low, 10, "T1499"},

    {MonAnomalyRule_ElevatedIoRingAbuse, L"ElevatedAbuse", 1, 0, MonSeverity_High, 25, "T1548"},

    {MonAnomalyRule_BurstPattern, L"BurstPattern", 500, 0, MonSeverity_Medium, 15, "T1499"},

    {MonAnomalyRule_ConcurrentTargets, L"ConcurrentFiles", 50, 60, MonSeverity_Medium, 15, "T1083"},

    {MonAnomalyRule_ViolationAccumulation, L"ViolationAccum", 5, 0, MonSeverity_High, 25, "T1068"},
};
 ...
 *-------------------------------------------------------------------------*/
static BOOLEAN MonAnomalyEvaluateSingleRule(_In_ const MON_ANOMALY_RULE *Rule,
                                            _In_ const MON_PROCESS_PROFILE *Profile,
                                            _In_ ULONG OpsPerSecond, _Out_ ULONG *ActualValue);

/*==========================================================================
 * Initialization and Shutdown
 *=========================================================================*/

_Use_decl_annotations_ NTSTATUS MonAnomalyInitialize(VOID) {
  ULONG i;
...
_Use_decl_annotations_ VOID MonAnomalyShutdown(VOID) {
  if (InterlockedCompareExchange(&g_AnomalyState.Initialized, 0, 0) == 0) {
    return;
  }
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
             ...
             g_AnomalyState.TotalEvaluations, g_AnomalyState.TotalMatches);
}

_Use_decl_annotations_ BOOLEAN MonAnomalyIsInitialized(VOID) {
...
_Use_decl_annotations_ ULONG MonAnomalyEvaluate(const MON_PROCESS_PROFILE *Profile,
                                                ULONG OpsPerSecond, ULONG *TriggeredRules,
                                                MON_ANOMALY_RESULT *Results, ULONG MaxResults) {
  ULONG matchCount = 0;
  ...
  *ActualValue = 0;
  ...
  case MonAnomalyRule_RapidHandleCreation:
    *ActualValue = Profile->TotalHandlesCreated;
    return (Profile->TotalHandlesCreated > Rule->Threshold && Profile->ActiveHandleCount > 5);
...
_Use_decl_annotations_ NTSTATUS MonAnomalyGetRule(MON_ANOMALY_RULE_ID RuleId,
...
_Use_decl_annotations_ NTSTATUS MonAnomalySetThreshold(MON_ANOMALY_RULE_ID RuleId,
                                                       ULONG Threshold) {
  ULONG i;
...
_Use_decl_annotations_ NTSTATUS MonAnomalyEnableRule(MON_ANOMALY_RULE_ID RuleId, BOOLEAN Enable) {
  ULONG i;
  ...
  for (i = 0; i < g_AnomalyState.RuleCount; i++) {
    if (g_AnomalyState.Rules[i].RuleId == RuleId) {
      ...
      DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][ANOMALY] Rule %lu %s\n",
...
_Use_decl_annotations_ NTSTATUS MonAnomalyAddRule(const MON_ANOMALY_RULE *NewRule) {
  ULONG i;
  ...
  if (NewRule->RuleId == MonAnomalyRule_None || NewRule->RuleId >= MonAnomalyRule_Max) {
    return STATUS_INVALID_PARAMETER;
  }
  ...
  /* Add rule */
  RtlCopyMemory(&g_AnomalyState.Rules[g_AnomalyState.RuleCount], NewRule, sizeof(MON_ANOMALY_RULE));
  g_AnomalyState.RuleCount++;

  ExReleaseFastMutex(&g_AnomalyState.RuleMutex);

  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][ANOMALY] Added rule %lu: %ws\n",
...
_Use_decl_annotations_ NTSTATUS MonAnomalyEnumerateRules(PMON_ANOMALY_RULE Buffer, ULONG MaxCount,
                                                         ULONG *ActualCount) {
  ULONG copyCount;
  ...
  *ActualCount = 0;
  ...
  if (copyCount > 0) {
    RtlCopyMemory(Buffer, g_AnomalyState.Rules, copyCount * sizeof(MON_ANOMALY_RULE));
  }
...
_Use_decl_annotations_ ULONG MonAnomalyGetRuleCount(VOID) {
  if (!MonAnomalyIsInitialized()) {
    return 0;
  }
...
_Use_decl_annotations_ VOID MonAnomalyGetStats(PMON_ANOMALY_STATS Stats) {
  if (Stats == NULL) {
    return;
  }
  ...
  Stats->EnabledRules = g_AnomalyState.RuleCount - (ULONG)g_AnomalyState.RulesDisabled;
  Stats->TotalEvaluations = (ULONG)g_AnomalyState.TotalEvaluations;
  Stats->TotalMatches = (ULONG)g_AnomalyState.TotalMatches;
}

_Use_decl_annotations_ VOID MonAnomalyResetStats(VOID) {
  if (!MonAnomalyIsInitialized()) {
    return;
  }
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][ANOMALY] Statistics reset\n");
}
