 ...
 */
_IRQL_requires_(PASSIVE_LEVEL) NTSTATUS MonMemMonitorInitialize(VOID);
 ...
 */
_IRQL_requires_(PASSIVE_LEVEL) VOID MonMemMonitorShutdown(VOID);
...
_IRQL_requires_max_(APC_LEVEL) NTSTATUS
    MonMemTrackMdl(_In_ ULONG ProcessId, _In_ PVOID MdlAddress, _In_opt_ ULONG64 IoRingHandle,
 ...
 * @thread-safety Uses ERESOURCE for list protection
 */
_IRQL_requires_max_(APC_LEVEL) VOID MonMemUntrackMdl(_In_ ULONG ProcessId, _In_ PVOID MdlAddress);
...
_IRQL_requires_(PASSIVE_LEVEL) NTSTATUS
    MonMemGetMdlTracker(_In_ ULONG ProcessId,
                        _Out_writes_bytes_to_(OutLen, *BytesWritten) PVOID OutBuffer,
                        _In_ ULONG OutLen, _Out_ ULONG *BytesWritten);
 ...
 */
_IRQL_requires_(PASSIVE_LEVEL) NTSTATUS
    MonMemScanVad(_In_ ULONG ProcessId,
 ...
 */
_IRQL_requires_(PASSIVE_LEVEL) NTSTATUS
    MonMemAnalyzePhysical(_In_ ULONG ProcessId,
 ...
 */
_IRQL_requires_(PASSIVE_LEVEL) NTSTATUS
    MonMemDetectSharing(_In_ ULONG ProcessId,
                        _Out_writes_bytes_to_(OutLen, *BytesWritten) PVOID OutBuffer,
                        _In_ ULONG OutLen, _Out_ ULONG *BytesWritten);
...
_IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS
    MonMemGetStats(_Out_writes_bytes_(sizeof(MON_MEM_STATS)) PVOID OutBuffer, _In_ ULONG OutLen);
 ...
 */
_IRQL_requires_(PASSIVE_LEVEL) ULONG MonMemCheckAnomalies(_In_ ULONG ProcessId);
