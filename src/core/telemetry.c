 ...
 * @purpose    Internal helper to allocate and queue an EVENT_BLOB if telemetry
 * is enabled
 * @precondition IRQL <= DISPATCH_LEVEL; PayloadLen <= MON_MAX_EVENT_BLOB_BYTES
 * @postcondition On success, pushes node to EventQueue and increments
 * EventCount; on failure increments DroppedEvents
 ...
 */
static VOID EnqueueEventBlob(_In_ MONITOR_EVENT_TYPE Type,
                             _In_reads_bytes_opt_(PayloadLen) const VOID *Payload,
                             _In_range_(0, MON_MAX_EVENT_BLOB_BYTES) ULONG PayloadLen) {
  if (!g_Mon.TelemetryEnabled)
    return;
  if (PayloadLen > MON_MAX_EVENT_BLOB_BYTES)
    return;
...
#if DBG
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][TEL] queued type=%u len=%lu\n",
             Type, PayloadLen);
#endif
}
 ...
 * @postcondition Returns STATUS_SUCCESS; no allocations beyond future ETW
 * registration
 ...
 * @purpose    Rate-limited logging: enqueues an event blob if within the
 * current window
 * @precondition IRQL <= DISPATCH_LEVEL; Payload is optional and fits within
 * configured max size
 * @postcondition Event enqueued or dropped; global rate counters updated
 * @thread-safety Lock-free via interlocked ops; global window maintained
 * atomically
 * @side-effects Updates DroppedEvents on over-limit
 *
 * B3 Enhancement: Now integrates per-process rate limiting via
 * MonRateLimitCheckEvent(). Events are dropped if either global or per-process
 * limits are exceeded.
 */
_IRQL_requires_max_(DISPATCH_LEVEL) VOID
  ...
  /* Legacy global rate limiting (fallback if B3 disabled or for backward
   * compat) */
  LARGE_INTEGER now;
   ...
   * - Ring buffer: Accessed via IOCTL_MONITOR_RINGBUF_SNAPSHOT
   * (non-destructive)
   */
