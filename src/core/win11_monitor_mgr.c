...
/* Core IOCTLs */
static NTSTATUS MonIoctlGetVersion(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlGetCaps(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlEnable(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlDisable(VOID);
static NTSTATUS MonIoctlGetStats(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlFetchEvents(_Out_ PVOID Out, _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlSetTelemetry(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlSetEncryption(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlScanNow(VOID);
static NTSTATUS MonIoctlParseIopMc(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                   _In_ ULONG OutLen);
static NTSTATUS MonIoctlGetOffsetStatus(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlGetIoRingHandles(_Out_ PVOID Out, _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlSetMaskPolicy(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlGetRateStats(_Out_ PVOID Out, _In_ ULONG OutLen);

/* Ring buffer IOCTLs (E1) */
static NTSTATUS MonIoctlRingBufConfigure(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlRingBufSnapshot(_Out_ PVOID Out, _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlRingBufGetStats(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlRingBufClear(VOID);

/* Interception IOCTLs (Phase 6) */
static NTSTATUS MonIoctlInterceptValidate(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                          _In_ ULONG OutLen);
static NTSTATUS MonIoctlInterceptSetPolicy(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlInterceptGetPolicy(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlInterceptGetStats(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlInterceptResetStats(VOID);
static NTSTATUS MonIoctlInterceptEnable(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlInterceptAddBlacklist(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlInterceptRemoveBlacklist(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlInterceptGetBlacklist(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                              _In_ ULONG OutLen, _Out_ ULONG *BytesOut);

/* Profile IOCTLs (Phase 7) */
static NTSTATUS MonIoctlProfileGet(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                   _In_ ULONG OutLen);
static NTSTATUS MonIoctlProfileList(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                    _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlProfileExportML(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                        _In_ ULONG OutLen);
static NTSTATUS MonIoctlProfileGetStats(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlProfileGetConfig(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlProfileSetConfig(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlProfileReset(VOID);

/* Anomaly IOCTLs (Phase 7) */
static NTSTATUS MonIoctlAnomalyGetRules(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                        _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlAnomalySetThreshold(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlAnomalyEnableRule(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlAnomalyGetStats(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlAnomalyResetStats(VOID);

/* Memory Monitoring IOCTLs (Phase 8) */
static NTSTATUS MonIoctlMemScanVad(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                   _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlMemGetMdls(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                   _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlMemScanPhysical(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                        _In_ ULONG OutLen);
static NTSTATUS MonIoctlMemGetSharing(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                      _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlMemGetStats(_Out_ PVOID Out, _In_ ULONG OutLen);

/* Cross-Process Detection IOCTLs (Phase 9) */
static NTSTATUS MonIoctlXpGetShared(_Out_ PVOID Out, _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlXpGetTree(_Out_ PVOID Out, _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlXpScanSections(_In_ PVOID In, _In_ ULONG InLen, _Out_ PVOID Out,
                                       _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlXpGetAlerts(_Out_ PVOID Out, _In_ ULONG OutLen, _Out_ ULONG *BytesOut);
static NTSTATUS MonIoctlXpGetStats(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlXpGetConfig(_Out_ PVOID Out, _In_ ULONG OutLen);
static NTSTATUS MonIoctlXpSetConfig(_In_ PVOID In, _In_ ULONG InLen);
static NTSTATUS MonIoctlXpScanNow(VOID);

/* Timer DPC -> schedules pool scan work */
_Function_class_(KDEFERRED_ROUTINE) static VOID
    MonScanDpc(_In_ KDPC *Dpc, _In_opt_ PVOID DeferredContext, _In_opt_ PVOID SysArg1,
               _In_opt_ PVOID SysArg2);

/* Work item to perform scan */
_Function_class_(IO_WORKITEM_ROUTINE) static VOID
    MonScanWorkItem(_In_ PDEVICE_OBJECT DeviceObject, _In_opt_ PVOID Context);
...
_Use_decl_annotations_ NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject,
  ...
  if (!NT_SUCCESS(status)) {
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL, "[WIN11MON] ETW init failed: 0x%08X\n",
               status);
  }
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON] Loaded v%d.%d.%d (caps=0x%X)\n",
             WIN11MON_VERSION_MAJOR, WIN11MON_VERSION_MINOR, WIN11MON_VERSION_BUILD,
             (WIN11MON_CAP_IOP_MC | WIN11MON_CAP_POOL_TRACK | WIN11MON_CAP_TELEMETRY |
              WIN11MON_CAP_RATE_LIMIT | WIN11MON_CAP_ENCRYPTION_STUB));
 ...
 * Unload
 *-------------------------------------------------------------------------*/
_Use_decl_annotations_ VOID MonDriverUnload(PDRIVER_OBJECT DriverObject) {
 ...
 *-------------------------------------------------------------------------*/
_Use_decl_annotations_ NTSTATUS MonIrpCreate(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
...
_Use_decl_annotations_ NTSTATUS MonIrpClose(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  UNREFERENCED_PARAMETER(DeviceObject);
  Irp->IoStatus.Status = STATUS_SUCCESS;
  Irp->IoStatus.Information = 0;
  IoCompleteRequest(Irp, IO_NO_INCREMENT);
 ...
 * @purpose    IOCTL dispatcher (METHOD_BUFFERED); validates buffer sizes and
 * sets exact bytesOut
 * @precondition IRQL == PASSIVE_LEVEL; DO_BUFFERED_IO set; uses SystemBuffer
 * @postcondition Output populated only on success; no persistent global state
 * change
 * @thread-safety Serialized by I/O manager per device; independent IRPs may run
 * concurrently
 * @side-effects Telemetry events may be enqueued on certain paths
 */
_Use_decl_annotations_ NTSTATUS MonIrpDeviceControl(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  UNREFERENCED_PARAMETER(DeviceObject);
...
static NTSTATUS MonIoctlGetVersion(PVOID Out, ULONG OutLen) {
  ...
  ULONG v = (WIN11MON_VERSION_MAJOR << 24) | (WIN11MON_VERSION_MINOR << 16) |
...
static NTSTATUS MonIoctlGetCaps(PVOID Out, ULONG OutLen) {
  ...
  ULONG caps = WIN11MON_CAP_IOP_MC | WIN11MON_CAP_POOL_TRACK | WIN11MON_CAP_TELEMETRY |
               WIN11MON_CAP_RATE_LIMIT | WIN11MON_CAP_ENCRYPTION_STUB;
...
static NTSTATUS MonIoctlEnable(PVOID In, ULONG InLen) {
  ...
  if (s->EnableMonitoring > 1 || s->EnableTelemetry > 1 || s->EnableEncryption > 1) {
    return STATUS_INVALID_PARAMETER;
  }
  ...
  g_Mon.RateLimitPerSec =
      (s->RateLimitPerSec == 0) ? MON_DEFAULT_RATE_LIMIT_PER_SEC : s->RateLimitPerSec;
...
static NTSTATUS MonIoctlGetStats(PVOID Out, ULONG OutLen) {
  if (Out == NULL)
    return STATUS_INVALID_PARAMETER;
  if (OutLen < sizeof(MONITOR_STATS))
    return STATUS_BUFFER_TOO_SMALL;
...
static NTSTATUS MonIoctlParseIopMc(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen) {
  if (In == NULL || Out == NULL || InLen < sizeof(IOP_MC_BUFFER_ENTRY) ||
  ...
  /* SECURITY: Treat In as hostile. IopQueryMcBufferEntryEx uses SEH and policy
   * flags internally. */
  NTSTATUS status =
      IopQueryMcBufferEntryEx((PIOP_MC_BUFFER_ENTRY)In, &info, IOP_MC_QF_MASK_ADDRESS);
  if (!NT_SUCCESS(status)) {
    return status;
  }
...
static NTSTATUS MonIoctlGetOffsetStatus(PVOID Out, ULONG OutLen) {
  if (Out == NULL)
    return STATUS_INVALID_PARAMETER;
  if (OutLen < sizeof(MON_OFFSET_STATUS_OUTPUT))
    return STATUS_BUFFER_TOO_SMALL;
...
static NTSTATUS MonIoctlRingBufConfigure(PVOID In, ULONG InLen) {
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
             ...
             input->BufferSizeBytes, MonRingBufferIsInitialized() ? "YES" : "NO");
...
static NTSTATUS MonIoctlInterceptValidate(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen) {
  ...
  NTSTATUS status = MonInterceptValidateSubmission((PMON_INTERCEPT_REQUEST)In, InLen, &response);
...
static NTSTATUS MonIoctlInterceptAddBlacklist(PVOID In, ULONG InLen) {
  ...
  return MonInterceptAddToBlacklist(input->ProcessId,
 ...
 */
static NTSTATUS MonIoctlInterceptGetBlacklist(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen,
                                              ULONG *BytesOut) {
  NTSTATUS status;
  ULONG maxEntries;
  ULONG entryCount = 0;
  ...
  *BytesOut = 0;
  ...
  status = MonInterceptEnumerateBlacklist((PMON_BLACKLIST_ENTRY)Out, maxEntries, &entryCount);
...
static BOOLEAN NTAPI MonIoRingEnumCallback(_In_ ULONG ProcessId, _In_ HANDLE HandleValue,
                                           _In_ PVOID ObjectAddress, _In_ ACCESS_MASK GrantedAccess,
  ...
  if (MonIsRegBuffersValidationAvailable() && ObjectAddress != NULL) {
    ...
    if (offsets != NULL) {
      __try {
        info->RegBuffersCount = *(PULONG)((PUCHAR)ObjectAddress + offsets->RegBuffersCountOffset);
...
static NTSTATUS MonIoctlGetIoRingHandles(PVOID Out, ULONG OutLen, ULONG *BytesOut) {
  *BytesOut = 0;
  ...
  PMON_IORING_HANDLE_INFO handleArray = (PMON_IORING_HANDLE_INFO)MonAllocatePoolPaged(
      maxHandles * sizeof(MON_IORING_HANDLE_INFO), MON_POOL_TAG);
  if (handleArray == NULL) {
...
static NTSTATUS MonIoctlProfileList(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen,
                                    ULONG *BytesOut) {
...
static NTSTATUS MonIoctlAnomalyGetRules(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen,
                                        ULONG *BytesOut) {
  ULONG maxCount, actualCount = 0;
  NTSTATUS status;
 ...
 */
static NTSTATUS MonIoctlMemScanVad(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen,
                                   ULONG *BytesOut) {
  ULONG processId;
  NTSTATUS status;
 ...
 */
static NTSTATUS MonIoctlMemGetMdls(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen,
                                   ULONG *BytesOut) {
  ULONG processId;
  ...
  *BytesOut = 0;
  ...
  /* Copy MDL info array */
  PMON_MDL_INFO_PUBLIC mdlArray =
      (PMON_MDL_INFO_PUBLIC)((PUCHAR)Out + sizeof(MON_MDL_TRACKER_PUBLIC));
  ...
  *BytesOut =
      sizeof(MON_MDL_TRACKER_PUBLIC) + (pubOut->MaxMdlsReturned * sizeof(MON_MDL_INFO_PUBLIC));
  return STATUS_SUCCESS;
}
...
static NTSTATUS MonIoctlMemScanPhysical(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen) {
  NTSTATUS status;
  ULONG processId;
  MON_PHYSICAL_SCAN_RESULT internalResult;
  MON_PHYSICAL_SCAN_RESULT_PUBLIC *pubOut;
  ...
  status = MonMemAnalyzePhysical(processId, &internalResult, sizeof(internalResult));
  if (!NT_SUCCESS(status)) {
    return status;
  }

  /* Map internal result to public structure */
  pubOut = (MON_PHYSICAL_SCAN_RESULT_PUBLIC *)Out;
  RtlZeroMemory(pubOut, sizeof(*pubOut));
  pubOut->Size = sizeof(MON_PHYSICAL_SCAN_RESULT_PUBLIC);
  pubOut->ProcessId = processId;
  pubOut->TotalPhysicalBytes = (ULONG64)internalResult.TotalPagesScanned * PAGE_SIZE;
  pubOut->PageCount = internalResult.TotalPagesScanned;
  pubOut->SharedPageCount = internalResult.SharedPagesFound;
  pubOut->DoubleMappedCount = internalResult.CrossProcessSharedPages;
  pubOut->SuspiciousCount = internalResult.SuspiciousSharedCount;
  pubOut->AnomalyFlags = internalResult.AnomalyFlags;
 ...
 */
static NTSTATUS MonIoctlMemGetSharing(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen,
                                      ULONG *BytesOut) {
  NTSTATUS status;
  ULONG processId;
  MON_SHARING_SCAN_RESULT internalResult;
  ULONG internalBytesWritten;
  MON_SHARING_SCAN_RESULT_PUBLIC *pubOut;
  ...
  *BytesOut = 0;
  ...
  /* Call core implementation */
  status = MonMemDetectSharing(processId, &internalResult, sizeof(internalResult),
                               &internalBytesWritten);
  if (!NT_SUCCESS(status)) {
    return status;
  }

  /* Map internal result to public structure */
  pubOut = (MON_SHARING_SCAN_RESULT_PUBLIC *)Out;
  RtlZeroMemory(pubOut, sizeof(*pubOut));
  pubOut->Size = sizeof(MON_SHARING_SCAN_RESULT_PUBLIC);
  pubOut->ProcessId = processId;
  pubOut->SharedRegionCount = internalResult.SharedRegionCount;
  pubOut->SuspiciousShareCount = internalResult.SuspiciousCount;
  pubOut->TotalSharedBytes = internalResult.TotalSharedBytes;
  pubOut->CrossProcessShareCount = internalResult.SharedRegionCount;
...
static NTSTATUS MonIoctlXpGetTree(PVOID Out, ULONG OutLen, ULONG *BytesOut) {
  ...
  *BytesOut = 0;
  ...
  MON_XP_PROCESS_ENTRY_PUBLIC *pubOut = (MON_XP_PROCESS_ENTRY_PUBLIC *)((PUCHAR)Out + headerSize);
 ...
 */
static NTSTATUS MonIoctlXpScanSections(PVOID In, ULONG InLen, PVOID Out, ULONG OutLen,
                                       ULONG *BytesOut) {
  ...
  *BytesOut = 0;
  ...
  if (OutLen < requiredSize) {
    if (sections)
      ExFreePoolWithTag(sections, MON_XP_TAG);
    return STATUS_BUFFER_TOO_SMALL;
  }
  ...
  for (ULONG i = 0; i < count; i++) {
    ...
    RtlCopyMemory(pubOut[i].SectionName, sections[i].SectionName, sizeof(pubOut[i].SectionName));
...
static NTSTATUS MonIoctlXpGetAlerts(PVOID Out, ULONG OutLen, ULONG *BytesOut) {
  ...
  *BytesOut = 0;
  ...
  if (OutLen < requiredSize) {
    if (alerts)
      ExFreePoolWithTag(alerts, MON_XP_TAG);
    return STATUS_BUFFER_TOO_SMALL;
  }
  ...
  MON_XP_ALERT_EVENT_PUBLIC *pubOut = (MON_XP_ALERT_EVENT_PUBLIC *)((PUCHAR)Out + headerSize);
...
/**
 * @function   MonScanDpc
 * @purpose    DPC callback that enqueues the pool scan work item at most once
 * until completion
 * @precondition IRQL == DISPATCH_LEVEL; DeferredContext is a valid
 * PMONITOR_CONTEXT
 * @postcondition Work item is queued if not already pending; allocation of
 * IoWorkItem may occur
 * @thread-safety Uses InterlockedCompareExchange on ScanWorkQueued to gate
 * enqueue
 ...
 * @purpose    Executes pool scan at PASSIVE_LEVEL and reschedules next scan
 * when enabled
 ...
 * @purpose    Validates and inspects IoRing registered buffer array entries;
 * emits telemetry
 * @precondition IRQL == PASSIVE_LEVEL; ArrayVirtualAddress readable for
 * ArrayByteLength bytes
 * @postcondition Increments policy counters and emits events on detections; no
 * persistent mutation otherwise
 ...
 */
_Use_decl_annotations_ NTSTATUS MonAnalyzeIoRingRegArray(PVOID ArrayVirtualAddress,
                                                         SIZE_T ArrayByteLength) {
  ...
  __try {
    ...
    for (SIZE_T i = 0; i < count; ++i) {
      ...
      ULONG violMask = 0;
      NTSTATUS vstatus = IopValidateMcBufferEntryEx((PIOP_MC_BUFFER_ENTRY)p, IOP_MC_VF_DEFAULT,
                                                    IOP_MC_KNOWN_TYPE_WIN11, &violMask);
...
static NTSTATUS MonCreateDevice(PDRIVER_OBJECT DriverObject) {
  ...
  NTSTATUS status =
      IoCreateDevice(DriverObject, 0, /* no extension */
                     &g_Mon.DeviceName, FILE_DEVICE_UNKNOWN, FILE_DEVICE_SECURE_OPEN, FALSE, &dev);
  if (!NT_SUCCESS(status))
    return status;
...
static VOID MonInitQueues(PMONITOR_CONTEXT Ctx) {
  ExInitializeNPagedLookasideList(&Ctx->EventLookaside, NULL, NULL, 0,
                                  sizeof(MON_EVENT_NODE) + MON_MAX_EVENT_BLOB_BYTES, MON_EVENT_TAG,
                                  0);
 ...
 * @purpose    Stops periodic scanning and drains pending work item before
 * shutdown
 * @precondition IRQL == PASSIVE_LEVEL; Ctx non-NULL
 * @postcondition Timer canceled, DPCs flushed, work item freed when inactive
 * @thread-safety Must be called in unload/teardown path; uses interlocked
 * gating
