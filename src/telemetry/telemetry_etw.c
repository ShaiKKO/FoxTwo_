 ...
 *   logman create trace Win11MonTrace -p
 * "{7E8B92A1-5C3D-4F2E-B8A9-1D2E3F4A5B6C}" -o trace.etl -ets logman stop
 * Win11MonTrace -ets
 */
...
TRACELOGGING_DEFINE_PROVIDER(g_hMonitorEtwProvider, MON_ETW_PROVIDER_NAME,
                             /* GUID: {7E8B92A1-5C3D-4F2E-B8A9-1D2E3F4A5B6C} */
                             (0x7e8b92a1, 0x5c3d, 0x4f2e, 0xb8, 0xa9, 0x1d, 0x2e, 0x3f, 0x4a, 0x5b,
                              0x6c));
#pragma data_seg()
 ...
 */
static FORCEINLINE VOID MonEtwWriteLegacy(
    _In_ USHORT EventId, _In_ UCHAR Level, _In_ ULONGLONG Keyword,
...
_Use_decl_annotations_ NTSTATUS MonEtwInitialize(VOID) {
  ...
  if (NT_SUCCESS(status)) {
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][ETW] Provider registered: %s\n",
  ...
  status = EtwRegister(&MON_ETW_PROVIDER_GUID, NULL, /* EnableCallback */
...
_Use_decl_annotations_ VOID MonEtwShutdown(VOID) {
  if (!g_EtwInitialized) {
    return;
  }
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][ETW] Provider unregistered\n");
}

_Use_decl_annotations_ BOOLEAN MonEtwIsEnabled(VOID) {
  if (!g_EtwInitialized) {
    return FALSE;
  }

#if MON_ETW_TRACELOGGING_AVAILABLE
  return TraceLoggingProviderEnabled(g_hMonitorEtwProvider, MON_ETW_LEVEL_VERBOSE,
                                     0 /* any keyword */
  );
#else
...
_Use_decl_annotations_ VOID MonEtwLogCrossVmDetection(ULONG ProcessId, ULONG ThreadId,
                                                      ULONG64 SuspectAddress, UCHAR Severity) {
  if (!g_EtwInitialized) {
    return;
  }
...
#if MON_ETW_TRACELOGGING_AVAILABLE
  TraceLoggingWrite(
      g_hMonitorEtwProvider, "CrossVmDetected", TraceLoggingLevel(MON_ETW_LEVEL_WARNING),
      TraceLoggingKeyword(MON_ETW_KEYWORD_DETECTION | MON_ETW_KEYWORD_IORING),
      TraceLoggingUInt32(ProcessId, "ProcessId"), TraceLoggingUInt32(ThreadId, "ThreadId"),
  ...
  MonEtwWriteLegacy((USHORT)MonEtwEvent_CrossVmDetected, MON_ETW_LEVEL_WARNING,
                    MON_ETW_KEYWORD_DETECTION | MON_ETW_KEYWORD_IORING, dataDesc, 4);
#endif

  /* Debug output in all builds for visibility */
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
             "[WIN11MON][ETW] CrossVmDetected: PID=%lu TID=%lu Addr=0x%llX Sev=%u\n", ProcessId,
             ThreadId, maskedAddress, Severity);
}

_Use_decl_annotations_ VOID MonEtwLogRegBuffersViolation(ULONG ProcessId, ULONG64 IoRingAddress,
...
#if MON_ETW_TRACELOGGING_AVAILABLE
  TraceLoggingWrite(g_hMonitorEtwProvider, "RegBuffersViolation",
  ...
  MonEtwWriteLegacy((USHORT)MonEtwEvent_RegBuffersViolation, MON_ETW_LEVEL_ERROR,
                    MON_ETW_KEYWORD_DETECTION | MON_ETW_KEYWORD_IORING, dataDesc, 4);
#endif
...
_Use_decl_annotations_ VOID MonEtwLogIoRingHandle(ULONG ProcessId, ULONG64 HandleValue,
                                                  ULONG64 ObjectAddress, ULONG AccessMask,
...
#if MON_ETW_TRACELOGGING_AVAILABLE
  TraceLoggingWrite(g_hMonitorEtwProvider, "IoRingHandleEvent", TraceLoggingLevel(level),
  ...
  MonEtwWriteLegacy((USHORT)EventType, level, MON_ETW_KEYWORD_IORING | MON_ETW_KEYWORD_HANDLE,
                    dataDesc, 5);
#endif
}

_Use_decl_annotations_ VOID MonEtwLogPoolAllocation(ULONG PoolTag, ULONG64 AllocationSize,
                                                    ULONG64 Address, BOOLEAN IsNew) {
  if (!g_EtwInitialized) {
    return;
  }
...
#if MON_ETW_TRACELOGGING_AVAILABLE
  TraceLoggingWrite(
      g_hMonitorEtwProvider, "PoolAllocation", TraceLoggingLevel(MON_ETW_LEVEL_VERBOSE),
      TraceLoggingKeyword(MON_ETW_KEYWORD_POOL), TraceLoggingUInt32(PoolTag, "PoolTag"),
      TraceLoggingUInt64(AllocationSize, "AllocationSize"),
      TraceLoggingHexUInt64(maskedAddress, "Address"), TraceLoggingBoolean(IsNew, "IsNew"));
#else
  /* Legacy path: Use direct EtwWrite */
  ...
  MonEtwWriteLegacy((USHORT)MonEtwEvent_PoolAllocation, MON_ETW_LEVEL_VERBOSE, MON_ETW_KEYWORD_POOL,
                    dataDesc, 4);
#endif
}

_Use_decl_annotations_ VOID MonEtwLogDriverEvent(MON_ETW_EVENT_ID EventId, ULONG Capabilities,
                                                 ULONG WindowsBuild) {
  ...
  case MonEtwEvent_DriverLoaded:
    TraceLoggingWrite(g_hMonitorEtwProvider, "DriverLoaded", TraceLoggingLevel(MON_ETW_LEVEL_INFO),
  ...
  case MonEtwEvent_DriverUnloaded:
    TraceLoggingWrite(g_hMonitorEtwProvider, "DriverUnloaded",
  ...
  case MonEtwEvent_MonitoringEnabled:
    TraceLoggingWrite(g_hMonitorEtwProvider, "MonitoringEnabled",
                      TraceLoggingLevel(MON_ETW_LEVEL_INFO),
                      TraceLoggingKeyword(MON_ETW_KEYWORD_TELEMETRY),
                      TraceLoggingUInt32((ULONG)EventId, "EventId"),
                      TraceLoggingHexUInt32(Capabilities, "Capabilities"),
  ...
  case MonEtwEvent_MonitoringDisabled:
    TraceLoggingWrite(g_hMonitorEtwProvider, "MonitoringDisabled",
                      TraceLoggingLevel(MON_ETW_LEVEL_INFO),
                      TraceLoggingKeyword(MON_ETW_KEYWORD_TELEMETRY),
                      TraceLoggingUInt32((ULONG)EventId, "EventId"),
                      TraceLoggingHexUInt32(Capabilities, "Capabilities"),
  ...
  default:
    TraceLoggingWrite(g_hMonitorEtwProvider, "DriverEvent", TraceLoggingLevel(MON_ETW_LEVEL_INFO),
                      TraceLoggingKeyword(MON_ETW_KEYWORD_TELEMETRY),
                      TraceLoggingUInt32((ULONG)EventId, "EventId"),
                      TraceLoggingHexUInt32(Capabilities, "Capabilities"),
                      TraceLoggingUInt32(WindowsBuild, "WindowsBuild"));
  ...
  MonEtwWriteLegacy((USHORT)EventId, MON_ETW_LEVEL_INFO, MON_ETW_KEYWORD_TELEMETRY, dataDesc, 3);
#endif
...
_Use_decl_annotations_ VOID MonEtwLogPoolSpray(ULONG PoolTag, const char *TagName,
                                               ULONG AllocationCount, ULONG Threshold,
                                               UCHAR Severity, const char *MitreTechnique) {
  if (!g_EtwInitialized) {
    return;
  }
...
#if MON_ETW_TRACELOGGING_AVAILABLE
  TraceLoggingWrite(
      g_hMonitorEtwProvider, "PoolSprayDetected", TraceLoggingLevel(level),
      TraceLoggingKeyword(MON_ETW_KEYWORD_DETECTION | MON_ETW_KEYWORD_POOL),
      TraceLoggingHexUInt32(PoolTag, "PoolTag"), TraceLoggingString(TagName, "TagName"),
      TraceLoggingUInt32(AllocationCount, "AllocationCount"),
      TraceLoggingUInt32(Threshold, "Threshold"), TraceLoggingUInt8(Severity, "Severity"),
  ...
  MonEtwWriteLegacy((USHORT)MonEtwEvent_PoolSprayDetected, level,
                    MON_ETW_KEYWORD_DETECTION | MON_ETW_KEYWORD_POOL, dataDesc, 4);
#endif

  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
             "[WIN11MON][ETW] PoolSprayDetected: Tag='%s' Count=%lu Threshold=%lu "
             "Sev=%u MITRE=%s\n",
             TagName ? TagName : "????", AllocationCount, Threshold, Severity,
             MitreTechnique ? MitreTechnique : "T1068");
}
