 ...
 */
DEFINE_GUID(MON_ETW_PROVIDER_GUID, 0x7e8b92a1, 0x5c3d, 0x4f2e, 0xb8, 0xa9, 0x1d, 0x2e, 0x3f, 0x4a,
            0x5b, 0x6c);
 ...
 */
_IRQL_requires_(PASSIVE_LEVEL) NTSTATUS MonEtwInitialize(VOID);
 ...
 */
_IRQL_requires_(PASSIVE_LEVEL) VOID MonEtwShutdown(VOID);
 ...
 * @side-effects None
 */
_IRQL_requires_max_(DISPATCH_LEVEL) BOOLEAN MonEtwIsEnabled(VOID);
...
_IRQL_requires_max_(DISPATCH_LEVEL) VOID
    MonEtwLogCrossVmDetection(_In_ ULONG ProcessId, _In_ ULONG ThreadId,
                              _In_ ULONG64 SuspectAddress, _In_ UCHAR Severity);
 ...
 */
_IRQL_requires_max_(DISPATCH_LEVEL) VOID
    MonEtwLogRegBuffersViolation(_In_ ULONG ProcessId, _In_ ULONG64 IoRingAddress,
                                 _In_ ULONG64 RegBuffersAddress, _In_ ULONG ViolationFlags);
 ...
 */
_IRQL_requires_max_(DISPATCH_LEVEL) VOID
    MonEtwLogIoRingHandle(_In_ ULONG ProcessId, _In_ ULONG64 HandleValue,
                          _In_ ULONG64 ObjectAddress, _In_ ULONG AccessMask,
 ...
 */
_IRQL_requires_max_(DISPATCH_LEVEL) VOID
    MonEtwLogPoolAllocation(_In_ ULONG PoolTag, _In_ ULONG64 AllocationSize, _In_ ULONG64 Address,
                            _In_ BOOLEAN IsNew);
 ...
 * @side-effects Emits ETW event if provider enabled
 */
_IRQL_requires_(PASSIVE_LEVEL) VOID
    MonEtwLogDriverEvent(_In_ MON_ETW_EVENT_ID EventId, _In_ ULONG Capabilities,
 ...
 */
_IRQL_requires_max_(DISPATCH_LEVEL) VOID
    MonEtwLogPoolSpray(_In_ ULONG PoolTag, _In_ const char *TagName, _In_ ULONG AllocationCount,
                       _In_ ULONG Threshold, _In_ UCHAR Severity, _In_ const char *MitreTechnique);
