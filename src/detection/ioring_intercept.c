 ...
 *
 * References:
 * - ziX-labs-c-style.md: 60-line function limit, SEH patterns, bounded
 * accessors
 ...
 */
static const CHAR *MonInterceptGetMitreTechnique(_In_ MON_INTERCEPT_REASON Reason) {
  switch (Reason) {
 ...
 */
static VOID MonInterceptInitResponse(_Out_ PMON_INTERCEPT_RESPONSE Response) {
 ...
 */
static VOID MonInterceptCapturePolicy(_Out_ PMON_INTERCEPT_POLICY Policy) {
 ...
 */
static BOOLEAN MonInterceptCheckBlacklist(_In_ ULONG ProcessId) {
 ...
 */
static MON_INTERCEPT_REASON MonInterceptValidateSingleSqe(_In_ const MON_SERIALIZED_SQE *Sqe,
  ...
  if (Policy->ValidateOpCodes && Policy->AllowedOpCodeMask != 0) {
    if (Sqe->OpCode > 31 || !(Policy->AllowedOpCodeMask & (1UL << Sqe->OpCode))) {
  ...
  if (Policy->MaxBufferSizeBytes != 0 && Sqe->BufferSize > Policy->MaxBufferSizeBytes) {
 ...
 */
static BOOLEAN MonInterceptCaptureSqe(_In_reads_bytes_(RequestSize) const VOID *UserBuffer,
                                      _In_ ULONG RequestSize, _In_ ULONG Index,
  ...
  /* Validate bounds */
  if (sqeOffset > RequestSize || (RequestSize - sqeOffset) < sizeof(MON_SERIALIZED_SQE)) {
    return FALSE;
  }

  /* Safe copy - caller has SEH protection */
  RtlCopyMemory(CapturedSqe, (const UCHAR *)UserBuffer + sqeOffset, sizeof(MON_SERIALIZED_SQE));
  return TRUE;
}
 ...
 */
static NTSTATUS MonInterceptValidateRequestHeader(_In_reads_bytes_(UserBufferSize)
 ...
 */
static NTSTATUS MonInterceptValidateRequestSize(_In_ ULONG UserBufferSize,
 ...
 */
static NTSTATUS MonInterceptCheckPolicyLimits(_In_ const MON_INTERCEPT_REQUEST *Request,
                                              _In_ const MON_INTERCEPT_POLICY *Policy,
                                              _Inout_ PMON_INTERCEPT_RESPONSE Response) {
  ...
  if (Policy->EnforceOperationLimit) {
    ...
    if (Request->OperationCount > maxOps) {
      ...
      RtlStringCchCopyA(Response->MitreTechnique, sizeof(Response->MitreTechnique),
 ...
 */
static NTSTATUS MonInterceptValidateSqeArray(_In_reads_bytes_(RequestSize) const VOID *UserBuffer,
                                             _In_ ULONG RequestSize, _In_ ULONG OperationCount,
                                             _In_ const MON_INTERCEPT_POLICY *Policy,
                                             _Inout_ PMON_INTERCEPT_RESPONSE Response) {
 ...
 */
static VOID MonInterceptLogEvent(_In_ const MON_INTERCEPT_REQUEST *Request,
  ...
  RtlCopyMemory(payload.Technique, Response->MitreTechnique, sizeof(payload.Technique));

  /* Write to telemetry ring buffer if available */
  if (MonRingBufferIsInitialized()) {
    MonRingBufferWrite(MonEvent_PolicyViolation, &payload, sizeof(payload));
  }

  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
             "[WIN11MON][INTERCEPT] %s PID=%lu Handle=0x%llX Reason=%u Tech=%s\n",
             Response->Action == MonIntercept_Block ? "Blocked" : "AuditLog", Request->ProcessId,
             Request->IoRingHandle, Response->Reason, Response->MitreTechnique);
}
 ...
 */
static VOID MonInterceptUpdateTimingStats(_In_ ULONG64 ValidationTimeNs) {
  InterlockedAdd64(&g_InterceptState.Stats.TotalValidationTimeNs, (LONG64)ValidationTimeNs);
  ...
  if (validationUs > currentPeak) {
    InterlockedCompareExchange((volatile LONG *)&g_InterceptState.Stats.PeakValidationTimeUs,
                               (LONG)validationUs, (LONG)currentPeak);
  }
}
...
_Use_decl_annotations_ NTSTATUS MonInterceptInitialize(VOID) {
...
_Use_decl_annotations_ VOID MonInterceptShutdown(VOID) {
  if (InterlockedCompareExchange(&g_InterceptState.Initialized, 0, 0) == 0) {
    return;
  }
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
             ...
             g_InterceptState.Stats.TotalValidationRequests, g_InterceptState.Stats.TotalBlocked);
}

_Use_decl_annotations_ BOOLEAN MonInterceptIsInitialized(VOID) {
 ...
 */
static VOID MonInterceptUpdateResultStats(_In_ MON_INTERCEPT_ACTION Action,
 ...
 */
static NTSTATUS MonInterceptValidateCore(_In_reads_bytes_(RequestSize) const VOID *Request,
  ...
  status = MonInterceptValidateRequestHeader((PMON_INTERCEPT_REQUEST)Request, RequestSize,
                                             &capturedHeader, Response);
  ...
  status = MonInterceptValidateRequestSize(RequestSize, capturedHeader.OperationCount, Response);
  if (!NT_SUCCESS(status)) {
    return status;
  }
  ...
  /* Validate each SQE */
  status = MonInterceptValidateSqeArray(Request, RequestSize, capturedHeader.OperationCount, Policy,
                                        Response);
...
_Use_decl_annotations_ NTSTATUS MonInterceptValidateSubmission(PMON_INTERCEPT_REQUEST Request,
                                                               ULONG RequestSize,
...
_Use_decl_annotations_ NTSTATUS MonInterceptSetPolicy(PMON_INTERCEPT_POLICY Policy) {
  KIRQL oldIrql;
...
_Use_decl_annotations_ VOID MonInterceptGetPolicy(PMON_INTERCEPT_POLICY Policy) {
  if (Policy == NULL) {
    return;
  }
...
_Use_decl_annotations_ VOID MonInterceptGetStats(PMON_INTERCEPT_STATS Stats) {
  if (Stats == NULL) {
    return;
  }
  ...
  if (Stats->TotalValidationRequests > 0) {
    Stats->AverageValidationTimeUs =
        (ULONG)((Stats->TotalValidationTimeNs / 1000) / Stats->TotalValidationRequests);
  }
}

_Use_decl_annotations_ VOID MonInterceptResetStats(VOID) {
  if (!MonInterceptIsInitialized()) {
    return;
  }
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][INTERCEPT] Statistics reset\n");
}

_Use_decl_annotations_ NTSTATUS MonInterceptAddToBlacklist(ULONG ProcessId, PCWSTR ProcessName,
                                                           PCSTR Reason) {
  KIRQL oldIrql;
    ...
    if (g_InterceptState.Blacklist[i].ProcessId == 0) {
      ...
      if (ProcessName != NULL) {
        RtlStringCchCopyW(g_InterceptState.Blacklist[i].ProcessName,
                          RTL_NUMBER_OF(g_InterceptState.Blacklist[i].ProcessName), ProcessName);
      }

      if (Reason != NULL) {
        RtlStringCchCopyA(g_InterceptState.Blacklist[i].Reason,
                          RTL_NUMBER_OF(g_InterceptState.Blacklist[i].Reason), Reason);
      }
...
_Use_decl_annotations_ BOOLEAN MonInterceptRemoveFromBlacklist(ULONG ProcessId) {
  KIRQL oldIrql;
  BOOLEAN found = FALSE;
  ...
  for (ULONG i = 0; i < MON_INTERCEPT_MAX_BLACKLIST; i++) {
    if (g_InterceptState.Blacklist[i].ProcessId == ProcessId) {
      RtlZeroMemory(&g_InterceptState.Blacklist[i], sizeof(MON_BLACKLIST_ENTRY));
...
_Use_decl_annotations_ BOOLEAN MonInterceptIsBlacklisted(ULONG ProcessId) {
  if (!MonInterceptIsInitialized()) {
    return FALSE;
  }
...
_Use_decl_annotations_ NTSTATUS MonInterceptEnumerateBlacklist(PMON_BLACKLIST_ENTRY Buffer,
                                                               ULONG MaxEntries,
                                                               ULONG *EntryCount) {
  KIRQL oldIrql;
  ULONG count = 0;
  ...
  *EntryCount = 0;
  ...
  for (ULONG i = 0; i < MON_INTERCEPT_MAX_BLACKLIST && count < MaxEntries; i++) {
    if (g_InterceptState.Blacklist[i].ProcessId != 0) {
      RtlCopyMemory(&Buffer[count], &g_InterceptState.Blacklist[i], sizeof(MON_BLACKLIST_ENTRY));
      count++;
    }
  }
...
_Use_decl_annotations_ BOOLEAN MonInterceptIsEnabled(VOID) {
  if (!MonInterceptIsInitialized()) {
    return FALSE;
  }
...
_Use_decl_annotations_ VOID MonInterceptEnable(BOOLEAN Enable) {
  KIRQL oldIrql;
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][INTERCEPT] %s\n",
