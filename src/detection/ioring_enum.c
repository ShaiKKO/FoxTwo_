 ...
 */
static const IORING_OFFSET_TABLE *MonFindOffsetsForBuild(_In_ ULONG BuildNumber) {
  ...
  if (MonOffsetResolverIsInitialized()) {
    ...
    if (NT_SUCCESS(status) && ioringOffsets.FieldCount >= 4) {
      ...
      DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
                 "[WIN11MON][IORING] Using offsets from resolver (build %lu, "
                 "source=%u)\n",
  ...
  for (ULONG i = 0; g_IoRingOffsets[i].BuildNumber != 0; i++) {
    ...
    if (delta >= -MON_IORING_BUILD_TOLERANCE && delta <= MON_IORING_BUILD_TOLERANCE) {
...
_Use_decl_annotations_ ULONG MonDetectWindowsBuild(VOID) {
...
_Use_decl_annotations_ NTSTATUS MonIoRingEnumInitialize(VOID) {
...
_Use_decl_annotations_ VOID MonIoRingEnumShutdown(VOID) {
  g_IoRingTypeInfo.Initialized = FALSE;
  g_CurrentOffsets = NULL;

  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][IORING] Shutdown complete\n");
}

_Use_decl_annotations_ const IORING_OFFSET_TABLE *MonGetIoRingOffsets(VOID) {
  return g_CurrentOffsets;
}

_Use_decl_annotations_ const MON_IORING_TYPE_INFO *MonGetIoRingTypeInfo(VOID) {
  return &g_IoRingTypeInfo;
}

_Use_decl_annotations_ NTSTATUS MonEnumerateIoRingObjects(PMON_IORING_CALLBACK Callback,
                                                          PVOID Context) {
  if (Callback == NULL) {
  ...
  handleInfo = (PSYSTEM_HANDLE_INFORMATION_EX)MonAllocatePoolPaged(bufferSize, MON_POOL_TAG);
  ...
  for (ULONG_PTR i = 0; i < handleInfo->NumberOfHandles; i++) {
    ...
    if (g_IoRingTypeInfo.TypeIndex != 0) {
       ...
       */
      if (!(violations & (MON_REGBUF_VF_ACCESS_VIOLATION | MON_REGBUF_VF_OFFSETS_UNAVAIL))) {
    ...
    if (isIoRingCandidate) {
      BOOLEAN continueEnum = Callback((ULONG)entry->UniqueProcessId, (HANDLE)entry->HandleValue,
                                      entry->Object, entry->GrantedAccess, Context);
