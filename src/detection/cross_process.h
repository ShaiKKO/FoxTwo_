 ...
 * Summary
 * -------
 * Cross-process communication detection focused on IoRing buffer sharing scenarios.
 * Detects shared buffers between processes, tracks handle inheritance/duplication,
 * and correlates named object access to identify potential data exfiltration or
...
BOOLEAN MonXpIsInitialized(VOID);
...
MonXpGetSharedObjects(
    _Outptr_result_maybenull_ PMON_XP_SHARED_OBJECT* Objects,
    _Out_ ULONG* Count
);
...
NTSTATUS
MonXpGetProcessTree(
    _Outptr_result_maybenull_ PMON_XP_PROCESS_ENTRY* Entries,
    _Out_ ULONG* Count
);
...
NTSTATUS
MonXpGetAlerts(
    _Outptr_result_maybenull_ PMON_XP_ALERT_EVENT* Alerts,
    _Out_ ULONG* Count
);
 ...
 * @param[out] Sections - Receives allocated array (caller frees with MON_XP_TAG)
...
MonXpScanSections(
    ...
    _Outptr_result_maybenull_ PMON_XP_SECTION_INFO* Sections,
    _Out_ ULONG* Count
);
...
MonXpConvertProcessEntryToPublic(
    _In_ const MON_XP_PROCESS_ENTRY* Internal,
    _Out_ PVOID Public
);
...
MonXpConvertAlertToPublic(
    _In_ const MON_XP_ALERT_EVENT* Internal,
    _Out_ PVOID Public
);
...
NTSTATUS
MonXpSetConfig(
    _In_ const MON_XP_CONFIG* Config
);
...
MonXpIsProcessDescendant(
    _In_ ULONG AncestorPid,
    _In_ ULONG DescendantPid,
    _In_ ULONG MaxDepth
);
