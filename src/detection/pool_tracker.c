/*
 * Pool Tracker – Big Pool scanning for target tags (e.g., 'IrRB', 'Wnf',
 * 'NpFr')
 *
...
static VOID MonCheckSprayThresholds(VOID) {
  ...
  for (ULONG i = 0; i < MON_TAG_COUNT; ++i) {
    ...
    BOOLEAN deltaSpike =
        (prevCount > 0) && (count >= prevCount * 2) && (count >= cfg->SprayThreshold / 2);
    ...
    if ((thresholdExceeded || deltaSpike) && !g_SprayState.SprayAlertSent[i]) {
      ...
      /* Emit ETW alert */
      MonEtwLogPoolSpray(cfg->Tag, cfg->TagName, count, cfg->SprayThreshold, severity,
                         cfg->MitreTechnique);
  ...
  RtlCopyMemory(g_SprayState.PrevTagCounts, g_SprayState.TagCounts, sizeof(g_SprayState.TagCounts));
...
_IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS
    MonPoolTrackerInitialize(_Inout_ PMONITOR_CONTEXT Ctx,
                             _In_ PFN_MON_ANALYZE_REGARRAY AnalyzeCb) {
 ...
 * @precondition IRQL <= DISPATCH_LEVEL; Ctx->ScanTimer and Ctx->ScanDpc
 * initialized
 ...
 * @purpose    Queries SystemBigPoolInformation and invokes analysis for
 * target-tag allocations
 * @precondition IRQL <= PASSIVE_LEVEL; Ctx non-NULL; g_AnalyzeCb installed
 * @postcondition Iterates big pool entries and invokes g_AnalyzeCb on matches;
 * frees temp buffer
 * @thread-safety Should be invoked from single work item; not re-entrant by
 * design
 * @side-effects Allocates/frees NonPagedPoolNx buffer; may enqueue telemetry
 * via callback
 *
...
_IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS MonPoolScanNow(_Inout_ PMONITOR_CONTEXT Ctx) {
   ...
   */
  ULONG cachedSize = InterlockedCompareExchange((volatile LONG *)&g_CachedBigPoolBufferSize, 0, 0);

  if (cachedSize > 0) {
    /* Try cached size first with margin for growth */
    ULONG trySize = cachedSize + MON_BIGPOOL_GROWTH_MARGIN;

    info = (PSYSTEM_BIGPOOL_INFORMATION)MonAllocatePoolNonPaged(trySize, MON_POOL_TAG);

    if (info != NULL) {
      status = ZwQuerySystemInformation(SystemBigPoolInformation, info, trySize, &bytes);
  ...
  info = (PSYSTEM_BIGPOOL_INFORMATION)MonAllocatePoolNonPaged(bytes, MON_POOL_TAG);
  if (!info)
...
ProcessEntries:
  ...
  for (ULONG i = 0; i < info->Count; ++i) {
    ...
    /* e->VirtualAddress may have low bit used to mark NonPaged in some builds;
     * mask it */
