 ...
 *-------------------------------------------------------------------------*/
static PMON_STRUCTURE_OFFSETS MonOffsetFindStructure(_In_z_ const CHAR *StructureName) {
 ...
 *-------------------------------------------------------------------------*/
static PMON_RESOLVED_OFFSET MonOffsetFindField(_In_ PMON_STRUCTURE_OFFSETS Structure,
 ...
 *-------------------------------------------------------------------------*/
static NTSTATUS MonOffsetResolveIoRingEmbedded(_In_ ULONG BuildNumber,
  ...
  for (ULONG i = 0; g_IoRingEmbeddedOffsets[i].BuildMin != 0; i++) {
    if (BuildNumber >= g_IoRingEmbeddedOffsets[i].BuildMin &&
      ...
      /* RegBuffersCount */
      RtlStringCchCopyA(Offsets->Fields[0].FieldName, MON_OFFSET_MAX_NAME_LEN,
                        MON_FIELD_REGBUFFERS_COUNT);
      ...
      /* RegBuffers */
      RtlStringCchCopyA(Offsets->Fields[1].FieldName, MON_OFFSET_MAX_NAME_LEN,
                        MON_FIELD_REGBUFFERS);
      ...
      /* RegFilesCount */
      RtlStringCchCopyA(Offsets->Fields[2].FieldName, MON_OFFSET_MAX_NAME_LEN,
                        MON_FIELD_REGFILES_COUNT);
      ...
      DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
                 "[WIN11MON][OFFSET] Resolved %s from embedded table (build "
                 "%lu->%lu)\n",
 ...
 *-------------------------------------------------------------------------*/
static NTSTATUS MonOffsetResolveIoRingInferred(_In_ ULONG BuildNumber, _In_ ULONG Tolerance,
                                               _Out_ PMON_STRUCTURE_OFFSETS Offsets) {
...
MonOffsetResolverInitialize(const MON_OFFSET_RESOLVER_CONFIG *Config) {
  NTSTATUS status;
  ...
  if (status == STATUS_NOT_FOUND && g_OffsetState.Config.EnableInference) {
    ...
    status = MonOffsetResolveIoRingInferred(g_OffsetState.CurrentBuild,
                                            g_OffsetState.Config.BuildTolerance, &ioringOffsets);
  }
  ...
  if (NT_SUCCESS(status)) {
    ...
    if (!NT_SUCCESS(status)) {
      DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
                 "[WIN11MON][OFFSET] Failed to register %s: 0x%08X\n", MON_STRUCT_IORING_OBJECT,
                 status);
    }
  } else {
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
               "[WIN11MON][OFFSET] No offsets for build %lu; operating in "
               "degraded mode\n",
               g_OffsetState.CurrentBuild);
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
             "[WIN11MON][OFFSET] Initialized (build=%lu, structures=%lu, "
             "degraded=%s)\n",
             g_OffsetState.CurrentBuild, g_OffsetState.StructureCount,
             g_OffsetState.Degraded ? "YES" : "NO");

  return STATUS_SUCCESS;
}

_Use_decl_annotations_ VOID MonOffsetResolverShutdown(VOID) {
  g_OffsetState.Initialized = FALSE;
  g_OffsetState.StructureCount = 0;

  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
             "[WIN11MON][OFFSET] Shutdown (embedded=%ld, inferred=%ld, "
             "failures=%ld)\n",
             g_OffsetState.EmbeddedHits, g_OffsetState.InferenceHits,
             g_OffsetState.ValidationFailures);
}

_Use_decl_annotations_ BOOLEAN MonOffsetResolverIsInitialized(VOID) {
  return g_OffsetState.Initialized;
}

_Use_decl_annotations_ NTSTATUS MonGetStructureOffsets(const CHAR *StructureName,
...
_Use_decl_annotations_ NTSTATUS MonGetFieldOffset(const CHAR *StructureName, const CHAR *FieldName,
                                                  PULONG Offset) {
...
_Use_decl_annotations_ NTSTATUS MonGetFieldOffsetWithSize(const CHAR *StructureName,
                                                          const CHAR *FieldName, PULONG Offset,
                                                          PULONG Size) {
...
_Use_decl_annotations_ MON_OFFSET_SOURCE MonGetOffsetSource(const CHAR *StructureName) {
...
_Use_decl_annotations_ BOOLEAN MonAreOffsetsValidated(const CHAR *StructureName) {
  if (StructureName == NULL || !g_OffsetState.Initialized) {
    return FALSE;
  }
...
_Use_decl_annotations_ NTSTATUS MonValidateStructureOffsets(const CHAR *StructureName,
                                                            PVOID TestObject) {
    ...
    __try {
      ...
      if (regBufCount > 0) {
        ...
        if (regBuffers != NULL && !MON_IS_KERNEL_ADDRESS(regBuffers)) {
          DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                     "[WIN11MON][OFFSET] Validation failed: RegBuffers=%p "
                     "(user-mode)\n",
                     regBuffers);
          structure->OverallValidation = MonOffsetValidation_Failed;
          InterlockedIncrement(&g_OffsetState.ValidationFailures);
...
_Use_decl_annotations_ VOID MonOffsetResolverGetStats(PMON_OFFSET_RESOLVER_STATS Stats) {
  if (Stats == NULL) {
    return;
  }
...
_Use_decl_annotations_ NTSTATUS MonRegisterStructureOffsets(const MON_STRUCTURE_OFFSETS *Offsets) {
  if (Offsets == NULL) {
    return STATUS_INVALID_PARAMETER;
  }
  ...
  RtlCopyMemory(&g_OffsetState.Structures[g_OffsetState.StructureCount], Offsets,
  ...
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
             ...
             Offsets->StructureName, Offsets->StructureSize, Offsets->FieldCount,
             Offsets->OverallSource);

  return STATUS_SUCCESS;
}

_Use_decl_annotations_ VOID MonOffsetResolverSetDegraded(BOOLEAN Degraded) {
  g_OffsetState.Degraded = Degraded;

  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[WIN11MON][OFFSET] Degraded mode %s\n",
