...
)
{
    ...
    for (entry = listHead->Flink; entry != listHead; entry = entry->Flink) {
        PMON_RATE_ENTRY rateEntry = CONTAINING_RECORD(
            entry, MON_RATE_ENTRY, HashLink);
 ...
 */
static ULONG
MonRateCalculateEffectiveCount(
    _In_ ULONG PrevCount,
    _In_ ULONG CurrentCount,
...
static VOID
MonRateUpdateWindow(
    _Inout_ PULONG PrevCount,
    _Inout_ PULONG CurrentCount,
 ...
 *
 * Reference: https://learn.microsoft.com/en-us/windows-hardware/drivers/kernel/timer-objects-and-dpcs
 *-------------------------------------------------------------------------*/

/**
 * @function   MonRateCleanupWorkRoutine
 * @purpose    Work item routine that performs stale entry cleanup at PASSIVE_LEVEL
 * @precondition IRQL == PASSIVE_LEVEL; called by system worker thread
 */
_Function_class_(IO_WORKITEM_ROUTINE)
static VOID
MonRateCleanupWorkRoutine(
 ...
 */
static VOID
MonRateLimitStartCleanupTimer(VOID)
{
 ...
 */
static VOID
MonRateLimitStopCleanupTimer(VOID)
{
    /* Signal shutdown */
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "[WIN11MON][RATE] Cleanup timer stopped\n");
}
...
NTSTATUS MonRateLimitInitialize(VOID)
{
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        ...
        g_RateState.GlobalLimitPerSec,
        g_RateState.PerProcessLimitPerSec);
...
VOID MonRateLimitShutdown(VOID)
{
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "[WIN11MON][RATE] Shutdown complete\n");
}
...
)
{
    ...
    InterlockedExchange((volatile LONG*)&g_RateState.GlobalLimitPerSec,
                        (LONG)GlobalLimitPerSec);
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        ...
        GlobalLimitPerSec, PerProcessLimitPerSec);
}
...
MON_RATE_RESULT MonRateLimitCheckEvent(_In_ ULONG ProcessId)
{
    ...
    MonRateUpdateWindow(
        &g_RateState.GlobalPrevCount,
        &g_RateState.GlobalCurrentCount,
        &g_RateState.GlobalWindowStart,
        currentTime
    );

    /* Check global limit first */
    elapsedMs = MonRateGetWindowElapsedMs(
        g_RateState.GlobalWindowStart, currentTime);
    effectiveGlobal = MonRateCalculateEffectiveCount(
        g_RateState.GlobalPrevCount,
        g_RateState.GlobalCurrentCount,
        elapsedMs
    );
    ...
    if (entry == NULL) {
        ...
        /* Create new entry */
        entry = (PMON_RATE_ENTRY)MonAllocatePoolNonPaged(
            sizeof(MON_RATE_ENTRY), MON_RATE_TAG);
    ...
    effectiveProcess = MonRateCalculateEffectiveCount(
        entry->PrevWindowCount,
        entry->CurrentWindowCount,
        elapsedMs
    );
    ...
    if (effectiveGlobal + 1 > g_RateState.PeakGlobalRate) {
        InterlockedExchange((volatile LONG*)&g_RateState.PeakGlobalRate,
                            (LONG)(effectiveGlobal + 1));
    }
...
VOID MonRateLimitCleanupStale(VOID)
{
    ...
    staleThreshold.QuadPart = currentTime.QuadPart -
        ((LONGLONG)MON_RATE_STALE_WINDOWS * MON_RATE_WINDOW_MS * 10000);
    ...
    if (removedCount > 0) {
        InterlockedAdd((volatile LONG*)&g_RateState.StaleRemoved,
                       (LONG)removedCount);
...
VOID MonRateLimitGetStats(_Out_ PMON_RATE_LIMIT_INTERNAL_STATS Stats)
{
    ...
    Stats->ActiveProcessCount = (ULONG)InterlockedCompareExchange(
        &g_RateState.ActiveProcesses, 0, 0);
    Stats->TotalEventsAllowed = InterlockedCompareExchange64(
        &g_RateState.TotalAllowed, 0, 0);
    Stats->TotalEventsDropped = InterlockedCompareExchange64(
        &g_RateState.TotalDropped, 0, 0);
    Stats->ProcessDropCount = InterlockedCompareExchange64(
        &g_RateState.ProcessDrops, 0, 0);
    Stats->GlobalDropCount = InterlockedCompareExchange64(
        &g_RateState.GlobalDrops, 0, 0);
...
VOID MonRateLimitResetStats(VOID)
{
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "[WIN11MON][RATE] Statistics reset\n");
}
